...
from tensorflow.python.keras.layers import Input, Dense, GRU, Embedding, CuDNNGRU, Dropout, Flatten
...
path_checkpoint = '23_1_checkpoint.keras'
...
df = pd.read_csv(r"data/1_5_Task_Cleaned_2FrameAugumented.csv", header=None)
...
df_target = df_target.drop(df_target.columns[88:], axis=1)
...
#x_train_scaled = np.around(x_train_scaled, decimals=2)
#x_test_scaled = np.around(x_test_scaled, decimals=2)

#y_train_scaled = np.around(y_train_scaled, decimals=2)
#y_test_scaled = np.around(y_test_scaled, decimals=2)

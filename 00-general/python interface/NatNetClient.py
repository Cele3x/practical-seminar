...
class NatNetClient:
    ...
    def __unpackRigidBody(self, data):
        ...
        if(self.computedHipFrame == False):
            ...
            self.csvFrame.append(pos[2])
            self.computedHipFrame = True
    ...
    def __unpackMocapData(self, data):
        ...
        self.csvFrame.append(frameNumber)
        ...
        self.csvWriter.writerow(self.csvFrame)
    ...
    def __unpackRigidBodyDescription(self, data):
        ...
        if (self.__natNetStreamVersion[0] >= 2):
            ...
            if(self.computedHipHeader == True):
                # Not Hip
                self.csvHeaderFrameBones.extend((name.decode('utf-8'), name.decode('utf-8'), name.decode('utf-8'), name.decode('utf-8')))
            else:
                # Is Hip
                self.csvHeaderFrameBones.extend((name.decode('utf-8'), name.decode('utf-8'), name.decode('utf-8'), name.decode('utf-8'), name.decode('utf-8'), name.decode('utf-8'), name.decode('utf-8')))
                self.computedHipHeader = True
    ...
    def __unpackSkeletonDescription(self, data):
        ...
        headerFrameCoordList = ["X", "Y", "Z", "W"]*rigidBodyCount
        self.csvHeaderFrameCoord.extend(headerFrameCoordList)
        return offset
    ...
    def __unpackDataDescriptions(self, data):
        ...
        # Write csv headers
        self.csvWriter.writerow(self.csvHeaderFrameBones)
        # self.csvWriter.writerow(self.csvHeaderFrameId)
        self.csvWriter.writerow(self.csvHeaderFrameCoord)
    ...
    def run(self):
        ...
        # Setup Csv logging
        self.csvHeaderFrameBones = ["Frame"]
        self.csvHeaderFrameCoord = [",","X", "Y", "Z"]
        self.computedHipHeader = False
        self.computedHipFrame = False
        self.csvFile = open(self.csvFileName, "w+", newline='')
        self.csvWriter = csv.writer(self.csvFile, delimiter=',', escapechar="/", quoting=csv.QUOTE_NONE)

...
class NatNetClient:
    ...
    def __replaceRigidbody(self, data):
        offset = 0
    ...
    def __unpackSkeleton(self, data, dataFull, offsetTotal):
        ...
        for j in range(0, rigidBodyCount):
            ...
            offsetChange = self.__unpackRigidBody(data[offset:])
            dataSliceRigidbodies = b"".join([dataSliceRigidbodies, self.__replaceRigidbody(data[offset:])])
            offset += offsetChange
    ...
    def __cmdThreadFunction(self, socket):
        while True:
            # Block for input
            data, addr = socket.recvfrom(32768)  # 32k byte buffer size
            if (len(data) > 0):
                self.commandSocket.sendto(data, (self.serverIPAddress, self.commandPort))
    ...
    def setInterceptStatus(self, status):
        ...
        print("Now Intercepting Data: ", status)
    ...
    def run(self):
        ...
        commandThread = Thread(target=self.__cmdThreadFunction, args=(self.commandSocket,))
        commandThread.start()
